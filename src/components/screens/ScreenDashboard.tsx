...
export function ScreenDashboard({ go }: { go: GoFn }) {
  const data = ALT_DATA;
  const [tab, setTab] = useState('home');
  const stats = { active: data.dashboard.length, closingThisWeek: 1, nextDeadline: 'Tue · Appraisal', days: 2 };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '0 20px' }}>
          {data.dashboard.map(tx => (
            <button
              ...
              onClick={() => tx.active ? go('upload') : undefined}
              style={{
