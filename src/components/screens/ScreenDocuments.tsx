...
import type { GoFn, DocItem } from '@/types';
...
export function ScreenDocuments({ go }: { go: GoFn }) {
  const d = ALT_DATA;
        ...
        {filter === 'all' && (
          <>
            ...
            <div className="alt-card" style={{ padding: 0, overflow: 'hidden' }}>
              {groups.na.map(doc => <DocRow key={doc.id} doc={doc}/>)}
            </div>
          </>
        )}
        {filter !== 'all' && (
