...
interface ScreenExtractingProps {
  go: GoFn;
  filename?: string;
  /** True once the backend extraction job has completed. Undefined in the walkthrough → self-completes. */
  ready?: boolean;
  /** Called when the animation finishes and the job is ready. Defaults to go('review'). */
  onComplete?: () => void;
}

export function ScreenExtracting({ go, filename, ready, onComplete }: ScreenExtractingProps) {
  const [progress, setProgress] = useState(0);
  const [step, setStep] = useState(0);

  // In the walkthrough there is no real job, so treat it as always ready.
  const jobReady = ready ?? true;

  // Keep the latest completion handler in a ref so the animation interval is
  // created once per readiness change rather than on every parent re-render.
  const completeRef = useRef<() => void>(() => go('review'));
  completeRef.current = onComplete ?? (() => go('review'));

  useEffect(() => {
    const t = setInterval(() => {
      setProgress(p => {
        // Hold just short of done until the real job reports ready, so the
        // animation reflects actual backend progress rather than faking it.
        const cap = jobReady ? 100 : 92;
        const n = Math.min(cap, p + 1.6);
        if (n >= 100) { clearInterval(t); setTimeout(() => completeRef.current(), 500); return 100; }
        return n;
      });
    }, 70);
  ...
  return (
    <div className="alt-screen-body" style={{
      ...
      <div style={{
        ...
        <span>{filename ?? 'contract.pdf'}</span>
      </div>
