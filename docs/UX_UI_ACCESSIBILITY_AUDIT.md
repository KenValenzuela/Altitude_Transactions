...
| --- | --- | --- | --- | --- |
| Dashboard loads into a skeleton-like state but has no explicit empty state. | Empty demos or API failures need clear recovery paths. | Add `EmptyState`, improve error retry guidance, and clarify upload CTA. | `frontend/src/app/dashboard/page.tsx`, feedback components | P0 |
| Transaction workspace has too many equally weighted sections. | Users cannot quickly identify what needs attention next. | Add an overview/action strip, group sections with `SectionHeader`, and prioritize review/tasks/deadlines. | `frontend/src/app/transactions/[id]/page.tsx`, workflow components | P0 |
